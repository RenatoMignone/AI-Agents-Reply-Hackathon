...
from tools_v2 import get_all_tools, get_flagged_transactions
...
def run_agent(
    ...
    profile_msg = (
        f"Optimization profile: {profile}.\n"
        "Scoring prioritizes economic impact and penalizes false positives.\n"
        f"Target output cardinality for this run: between {min_flags} and {max_flags} transactions.\n"
        "Prefer high-value, high-confidence frauds with multiple corroborating signals.\n"
        "Do not include routine salary/rent or highly recurrent benign payments."
    )

    low_recall_message = (
        f"Refine analysis for '{dataset_name}'.\n"
        f"Flagged set is below target ({min_flags}).\n"
        "Add only medium/high-confidence fraud IDs with economic relevance"
        " and at least two anomaly signals (amount/timing/location/recipient/communications)."
    )

    high_fp_message = (
        f"Refine analysis for '{dataset_name}'.\n"
        f"Flagged set is above target ({max_flags}).\n"
        "Prune aggressively: keep only strongest economically relevant fraud IDs."
    )
    ...
    if flagged and len(flagged) < min_flags and profile != "precision":
        if verbose:
            print(f"\n[Agent] {len(flagged)} flagged (<{min_flags}). Triggering recall refinement...")
        _stream_run(low_recall_message)
        flagged = get_flagged_transactions()

    if flagged and len(flagged) > max_flags:
        if verbose:
            print(f"\n[Agent] {len(flagged)} flagged (>{max_flags}). Triggering precision refinement...")
        _stream_run(high_fp_message)
        flagged = get_flagged_transactions()
